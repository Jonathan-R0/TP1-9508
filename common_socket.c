...
int socket_destroy(socket_t* socket){
	if (socket == NULL ||
		shutdown(socket->fd, SHUT_RDWR) == -1 ||
		close(socket->fd) == -1) return -1;
	return 0;
}
...
int socket_accept(socket_t* socket, socket_t* client){
    ...
    int newSocket = accept(socket->fd, (struct sockaddr *)&address,
                           &addressLength);
...
/*
 * Función auxiliar que realiza el getaddrinfo(3) en función de los parámetros ingresados.
 * Devuelve una lista de posibles conexiones válidas. 
 * Precondiciones: socket != NULL && port != NULL.
 * Postcondiciones: devuelve la lista de direcciones, donde el pasaje de información será mediante un
 * stream continuo de datos. Devuelve NULL en caso de error, el struct addrinfo conseguido de lo contrario
 */
static struct addrinfo *get_addr_info(socket_t* socket, const char* port, const char* ip){
	...
	return address_list; // Es válido devolver este puntero ya que vive en el heap.
}
